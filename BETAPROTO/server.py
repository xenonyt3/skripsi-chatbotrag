...
from app import build_index, answer_extractive, answer_ollama

app = FastAPI()

# ---- CORS (bolehkan akses dari mana saja; sesuaikan kalau perlu)
app.add_middleware(
    CORSMiddleware,
    allow_origins=["*"],  # ganti ke domain kamu bila sudah pasti
    allow_credentials=True,
    allow_methods=["*"],
    allow_headers=["*"],
)

# ---- Static (opsional, kalau kamu punya UI di /static/index.html)
...
class ChatReq(BaseModel):
    ...
    top_k: int = 3
    model: str = "gpt-oss:20b"      # contoh default model kecil
...
# ---- Root: kalau ada static/index.html, layani itu
@app.get("/")
def root():
...
def chat(r: ChatReq):
    ...
    # jangan bikin FastAPI 500 tanpa pesan; tangkap error umum
    except Exception as e:
